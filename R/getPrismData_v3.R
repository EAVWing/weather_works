...
# Update calendar
url <- "http://www.prism.oregonstate.edu/calendar/list.php"
updates <- url %>%
  read_html() %>%
...
updatesClean <- updates %>%
  mutate(date = as_date(updates$Date, format = '%d %B %Y'),
...
# Domo init
DomoR::init(Sys.getenv("domoCustomer"), Sys.getenv("domoAccessToken"))
...
# start_date <- date("2019/01/01") #max(stableMean$Date)
# 
# end_date <- Sys.Date() - 1
# 
...
precip_dates_to_pull <- stable_precip_check$date
...
min_dates_to_pull <- stable_min_check$date
...
max_dates_to_pull <- stable_max_check$date
...
mean_dates_to_pull <- stable_mean_check$date
